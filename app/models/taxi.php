...
class Taxi extends AppModel {
	...
	var $virtualFields = array('point_as_text' => 'AsText(Taxi.position)',
								   'point_as_kml' => 'ST_AsKML(Taxi.position)');
	
	function afterFind($results) {
		foreach ($results as $key => $val):
		if (isset($val['Taxi']['point_as_text'])){
			$postgisPos = $val['Taxi']['point_as_text'];
			$csvLatLng = $this->convertToCsv($postgisPos);
			unset($val['Taxi']['point_as_text']);
